...
function modify_thermal_csv(capacity_constraint, technology)
    df = CSV.read("backup_systems/03_real_demand_sec_batteries/resources/Thermal.csv", DataFrame)
    df[!, "Max_Cap_MW"] .= 0  # Reset all "Max_Cap_MW" values
...
for capacity_constraint in capacity_constraints
    ...
    modify_thermal_csv(capacity_constraint, technology)
    ...
    capacity_dual_value = filter(:Resource => ==(technology), dfCapacity)[:, :Capacity_Constraint_Dual][1] 
