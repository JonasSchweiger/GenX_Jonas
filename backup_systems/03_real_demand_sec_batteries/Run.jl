...
#to watch out for: emergency period enabled or not, emergency quantity defined or not, constraint on replacement emissions, is solar switched on/off
...
@constraint(EP, cBackup_Total_Emissions, EP[:eBackup_Total_EReplacement] <= 42.14377237) # GenX.emergency_quantity_mmbtu(gen[8])) #value.(myinputs["dfMaxCO2"])
